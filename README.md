...
- 投稿参数已按需求固定：**分区 = 生活-日常**、**标题 = 【xxxx年x月x日录播】蕾蕾的直播回放**、**创作声明内容无需标注**
...
其中 `{date}` 会被替换为 `2026年9月24日` 这样的中文日期，`{name}` 会被替换为主播名。
你可以在 GUI 的「标题模板」输入框中自定义，例如：`{date} {name} 直播录像`。
...
3. 用 Inno Setup 打开 `packaging\bili_recorder.iss` 编译，或命令行：
   ...
   ```
4. 产物: `dist\BiliStreamRecorder-Setup-1.0.0.exe`
...
3. 执行：
   ...
   light.exe dist\obj\bili_recorder.wixobj -o dist\BiliStreamRecorder-Setup-1.0.0.msi
   ```
4. 产物: `dist\BiliStreamRecorder-Setup-1.0.0.msi`
